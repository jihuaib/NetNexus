...
服务器与客户端合并在一个三栏工作区里，布局与 Postman / BloomRPC 等 gRPC 工具一致：**左树选方法 → 中间编请求 → 右侧看响应**。顶栏切换模式并承载连接/监听参数。

```
┌ 模式: (客户端 | 服务器)  地址/端口  [TLS] [Metadata|解码规则|Unary回复]  [调用 / 启动服务器]  [独立监控窗口] ┐
├──────────────┬────────────────────────────────┬──────────────────────────────────────────┤
│ 服务 / 方法树  │ 客户端：请求 JSON（自动模板）        │ 响应 / 消息时间线：每条可展开 JSON 与原始字节  │
│ （服务器模式    │ 服务器：设备 / 活动流 + 下发编辑框    │ 客户端：按调用记录筛选，显示状态码 / Headers   │
│  勾选托管服务） │                                │ 服务器：按选中的流筛选                        │
└──────────────┴────────────────────────────────┴──────────────────────────────────────────┘
```
...
| 位置 | 说明 |
| --- | --- |
| 顶栏 | 目标 `host:port`、TLS（弹窗：CA / 客户端证书 / 私钥 / Server Name）、Metadata（弹窗键值对，`-bin` 结尾按 base64）、超时、`调用`。 |
| 左树 | 点击方法即选中；右键：查看属性、复制全名、生成请求模板。 |
| 中栏 | 请求 JSON；`生成模板` 按请求消息定义生成骨架；流式方法发起后出现 `发送消息 / 结束发送 / 取消调用`。 |
| 右栏 | 选择调用记录查看状态码、`Headers`（响应 metadata / trailers）与请求/响应时间线。 |

**服务器（上报）模式**

| 位置 | 说明 |
| --- | --- |
| 顶栏 | 监听地址/端口、TLS 与最大消息长度（弹窗）、解码规则（弹窗，可选）、Unary 回复模板（弹窗）、`启动 / 停止服务器`、监听状态。 |
| 左树 | 勾选要托管的服务（可多选）。 |
| 中栏 | 设备 / 活动流列表（对端、方法、收发计数）；选中一条流后在下方编辑 JSON `下发`，或 `关闭流`。 |
| 右栏 | 选中流的消息时间线；未选中时显示服务端全部消息。 |
