...
这几个页面都在看 BMP 路由，但回答的问题不同。所有页面都基于 current RIB 快照：SQLite 只保存每个 Scope 当前（active/stale）的路由，不保存历史事件流水。
...
| 路由追踪 | 一个 Prefix / NLRI 在 Pre-In 到 Post-Out 各阶段是什么样？ | 当前/ stale 路由，按五阶段横向组装 | 不能；成功撤销的路由不再保存 |
| 会话/Loc-RIB 统计 | 设备上报的 Statistics Report 当前值是什么？ | Statistics Report 最新投影 | 不适用 |

典型排障顺序是：先在“路由矩阵”发现异常候选，点击“追踪”进入“路由追踪”核对五阶段，再打开路由卡片详情查看完整属性。
...
- 只保存 current 投影：数据库回答“现在有没有”；已撤销路由随即从 RIB 投影删除，不会伪装成 current row，也不保留历史事件。
...
Session 和 Loc-RIB 的详情入口都来自 current route。路由成功 withdraw 或 purge 且没有重新宣告后，不会继续留在列表中，数据库也不再保留它的记录。
...
分析是在 BMP Worker 内对 SQLite 的 current RIB 做一次**流式扫描**：数据库按“同一 Client、同一 NLRI”的顺序吐出路由，Worker 逐条 NLRI 评估五阶段并只保留异常、计数和筛选面板，不会把整张 RIB 装进内存，百万级路由也能完成。扫描期间页面显示进度；邻居正在全表上报时，重建会等写入队列空闲后再开始，避免每次 EOR 都重头扫描。异常明细每类最多保留前 20,000 条（页面会提示），异常计数和漏斗始终是全量统计。分析开启后，后续的单条路由变化只重新评估受影响的 NLRI；Peer Up/Down、EOR 等 Scope 级变化则触发一次重建。
...
- 成功 withdraw 后 current row 已删除，不会继续出现在追踪结果中。
...
页面上没有一张“全字段路由表”。current 路由在查询时由 Scope、current 分区行、NLRI identity、扩展 payload、Path Attributes、Source 和 Connection 组装。
