<template>
    <nn-file-import-modal
        ...
        description="选择本地 ASPA JSON 文件，并设置本次最多处理的 ASPA 条数。"
        select-text="选择 ASPA JSON 文件"
        empty-text="尚未选择文件，请点击上方按钮选择一个 JSON 文件。"
        invalid-message="请先选择 ASPA JSON 文件"
        :loading="importing"
...
<script setup>
    ...
    const validateImport = filePath => {
        if (!filePath) return '请先选择 ASPA JSON 文件';
        if (!filePath.toLowerCase().endsWith('.json')) return '请选择扩展名为 .json 的 ASPA 文件';
