...
const { YANG_REQ_TYPES: WORKER_REQ_TYPES } = require('../../utils/yang');
...
class YangRuntimeService {
    constructor(ipcMain, store, options = {}) {
        this.ipcMain = ipcMain;
        this.store = store;
        this.rootDir = path.resolve(options.rootDir || path.join(process.cwd(), 'yang'));
        ...
        this.workerClient = options.compilerClient || new LocalYangCompilerClient();
        if (!options.compilerClient) {
            this.workerClient.configure({
        ...
        this.workerClient?.on?.('event', (eventName, data = {}) => {
            if (!['yang:compile-progress', 'yang:import-progress'].includes(eventName)) return;
        ...
        this.compilationRestoreControllers = new Map();
        this.activeCompilationRestores = new Map();
    ...
    invalidateCompilation(workspaceId) {
        ...
        for (const key of this.compilationRestorePromises.keys()) {
            if (key.startsWith(`${workspaceId}\u0000`)) this.compilationRestorePromises.delete(key);
        }
        for (const key of this.compilationRestoreControllers.keys()) {
            if (key.startsWith(`${workspaceId}\u0000`)) this.compilationRestoreControllers.delete(key);
        }
        this.persistCompileState(workspaceId, null, null);
    }

    abortCompilationRestores(workspaceId = null) {
        const pending = new Set();
        const matches = value => !workspaceId || value === workspaceId;
        for (const [promise, restore] of this.activeCompilationRestores) {
            if (!matches(restore.workspaceId)) continue;
            if (!restore.controller.signal.aborted) restore.controller.abort();
            pending.add(promise);
        }
        const prefix = workspaceId ? `${workspaceId}\u0000` : null;
        for (const [key, promise] of this.compilationRestorePromises) {
            if (!prefix || key.startsWith(prefix)) pending.add(promise);
        }
        return [...pending];
    }
    ...
    ensureWorker(event) {
        ...
        const error = new Error('YANG process compiler runtime is not configured');
            ...
            const task = this.taskManager.start(
                ...
                async ({ taskId, signal, report }) => {
                    ...
                    try {
                        ...
                        if (signal.aborted) {
                            throw this.lifecycleError('WORKER_CANCELLED', 'YANG编译已取消');
                        }
                        const workspace = await this.send(event, WORKER_REQ_TYPES.GET_WORKSPACE, {
                            workspaceId: context.workspaceId
                        });
    ...
    async restoreStoredCompilation(event, context, workspace, requestedCompileId, options = {}) {
        ...
        const restorePromise = (async () => {
            ...
            try {
                restored = await this.send(
                    ...
                    { timeoutMs: 10 * 60 * 1000, signal: restoreSignal }
                );
            } catch (error) {
                if (restoreSignal.aborted) {
                    throw this.lifecycleError('WORKER_CANCELLED', 'YANG编译上下文恢复已取消');
                }
                throw error;
            }
            if (restoreSignal.aborted) {
            ...
            const latestWorkspace = await this.send(
                event,
                WORKER_REQ_TYPES.GET_WORKSPACE,
                { workspaceId: context.workspaceId },
        ...
        this.compilationRestoreControllers.set(restoreKey, restoreController);
        this.activeCompilationRestores.set(restorePromise, {
            key: restoreKey,
            workspaceId: context.workspaceId,
        ...
        try {
            ...
            if (this.compilationRestoreControllers.get(restoreKey) === restoreController) {
                this.compilationRestoreControllers.delete(restoreKey);
            }
            this.activeCompilationRestores.delete(restorePromise);
        }
    }
    ...
    async deleteProfileWorkspace(profileId, event = null) {
        ...
        this.deletingWorkspaces.add(context.workspaceId);
        try {
            const pending = this.abortCompilationRestores(context.workspaceId);
    ...
    async close() {
        ...
        const closePromise = (async () => {
            ...
            const pendingRestores = this.abortCompilationRestores();
            await Promise.allSettled([...new Set([...pendingTasks, ...pendingRestores])]);
            ...
            this.compilationRestorePromises.clear();
            this.compilationRestoreControllers.clear();
            this.activeCompilationRestores.clear();
...
module.exports = YangRuntimeService;
